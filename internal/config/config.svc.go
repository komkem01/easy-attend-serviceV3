...
func newService[T any](dConf *T) *Service[T] {
	...
	appName := reflect.ValueOf(conf).Elem().FieldByName("AppName").String()
	if hostname == "" {
		hostname = appName + "-" + hex.EncodeToString(big.NewInt(rand.Int63()).Bytes())
	}
	debug := reflect.ValueOf(conf).Elem().FieldByName("Debug").Bool()
	appEnv := reflect.ValueOf(conf).Elem().FieldByName("AppEnv").String()
	return &Service[T]{
		hostname: hostname,
		appName:  appName,
		appEnv:   appEnv,
...
func (s *Service[T]) AppVersion() string {
	return Version
}

func (s *Service[T]) AppEnv() string {
	return s.appEnv
}
