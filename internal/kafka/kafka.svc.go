...
func newService(confVal *dto.Kafka) *Service {
	log := log.Default()
	...
	svc.InitTopics(confVal.Topics)

	return svc
}

func (s *Service) InitTopics(topics []string) {
	if s == nil {
		return
	}

	for _, topic := range topics {
		exist, err := s.ExistsTopic(context.Background(), topic)
		if err != nil {
			panic(err)
		}

		if !exist {
			if err := s.CreateTopic(context.Background(), topic); err != nil {
				panic(err)
			}
		}
	}
